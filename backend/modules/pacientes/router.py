...
@router.get("/buscar", response_model=schemas.Cliente)
def buscar_paciente(telefono: str, db: Session = Depends(get_db)):
    telefono_limpio = telefono.replace("+", "").strip()
    cliente = db.query(models.Cliente).filter(
        models.Cliente.telefono.like(f"%{telefono_limpio}%")
    ).first()
