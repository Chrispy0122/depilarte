...
def crear_cita(
    ...
    # 3.5 Validación de Solapamiento
    citas_solapadas = db.query(models.Cita).filter(
        models.Cita.fecha_hora_inicio < fecha_fin,
        models.Cita.fecha_hora_fin > cita.fecha_hora_inicio
    ).all()

    if citas_solapadas:
        for solap in citas_solapadas:
            # Check for exact duplicate (same client, same time +/- 60s)
            if solap.cliente_id == cita.cliente_id and abs((solap.fecha_hora_inicio - cita.fecha_hora_inicio).total_seconds()) < 60:
                raise HTTPException(status_code=409, detail=f"Cita ya registrada con ID: {solap.id}")
        
        # If we reach here, it's a conflict with another appointment (or same client's overlapping appointment)
        raise HTTPException(status_code=400, detail="Horario ocupado, existe solapamiento con otra cita.")
...
def listar_citas(fecha: Optional[str] = None, db: Session = Depends(get_db)):
    ...
    if fecha:
        try:
            ...
            query = query.filter(func.date(models.Cita.fecha_hora_inicio) == target_date)
        except ValueError:
            pass
    return query.all()
...
def list_services(
    ...
    """
    Lista todos los servicios filtrados por el negocio_id autenticado.
    """
    return db.query(models.Servicio).filter(models.Servicio.negocio_id == usuario_actual.negocio_id).all()

# ENDPOINT DESHABILITADO: El catálogo de servicios ahora se carga desde producción.
# Este endpoint generaba un Error 500 (IntegrityError: Column 'negocio_id' cannot be null)
